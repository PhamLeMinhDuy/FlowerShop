import React, { Component } from 'react';
import MyContext from '../contexts/MyContext';
import CartUtil from '../utils/CartUtil';
import axios from 'axios';
import withRouter from '../utils/withRouter';
import "./css/MyCartComponent.css"


class Mycart extends Component {
  static contextType = MyContext; // using this.context to access global state
  render() {
    const mycart = this.context.mycart.map((item, index) => {
      return (
        <tr key={item.plant._id} className="datatable">
          <td>{index + 1}</td>
          <td>{item.plant._id}</td>
          <td>{item.plant.name}</td>
          <td>{item.plant.category.name}</td>
          <td><img src={"data:image/jpg;base64," + item.plant.image} width="70px" height="70px" alt="" /></td>
          <td>{item.plant.price}</td>
          <td>{item.quantity}</td>
          <td>{item.plant.price * item.quantity}</td>
          <td><span className="link" onClick={() => this.lnkRemoveClick(item.product._id)}>Remove</span></td>
        </tr>
      );
    });
    return (
      <div className="cart">
        <h2 className="cart__title">Giỏ hàng</h2>
        <table className="cart__table" border="1">
          <tbody className="cart__table-body">
            <tr className="cart__table-body-title">
              <th>No.</th>
              <th>ID</th>
              <th>Name</th>
              <th>Category</th>
              <th>Image</th>
              <th>Price</th>
              <th>Quantity</th>
              <th>Amount</th>
              <th>Action</th>
            </tr>
            {mycart}
            <tr className="cart__table-body-row">
              <td colSpan="6"></td>
              <td>Total</td>
              <td>{CartUtil.getTotal(this.context.mycart)}</td>
              <td className="cart__table-body-row-btn"><span className="link" onClick={() => this.lnkCheckoutClick()}>Đặt</span></td>
            </tr>
          </tbody>
        </table>
      </div>
    );
  }
  // event-handlers
  lnkRemoveClick(id) {
    const mycart = this.context.mycart;
    const index = mycart.findIndex(x => x.product._id === id);
    if (index !== -1) { // found, remove item
      mycart.splice(index, 1);
      this.context.setMycart(mycart);
    }
  }
  lnkCheckoutClick() {
    if (window.confirm('Xác nhận đặt hàng?')) {
      if (this.context.mycart.length > 0) {
        const total = CartUtil.getTotal(this.context.mycart);
        const items = this.context.mycart;
        const customer = this.context.customer;
        if (customer) {
          this.apiCheckout(total, items, customer);
        } else {
          this.props.navigate('/login');
        }
      } else {
        alert('Giỏ hoa của bạn còn trống');
      }
    }
  }
  // apis
  apiCheckout(total, items, customer) {
    const body = { total: total, items: items, customer: customer };
    const config = { headers: { 'x-access-token': this.context.token } };
    axios.post('/api/customer/checkout', body, config).then((res) => {
      const result = res.data;
      if (result) {
        alert('Đặt thành công');
        this.context.setMycart([]);
        this.props.navigate('/home');
      } else {
        alert('Đặt thất bại');
      }
    });
  }
}
export default withRouter(Mycart);